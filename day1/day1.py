...
#Upon pouring into the office, everyone confirms that the Chief Historian is indeed nowhere to be found. Instead, the Elves discover an assortment of
#notes and lists of historically significant locations! This seems to be the planning the Chief Historian was doing before he left.
...
def count_list(list_to_count):
    ret_dict = {}
    for id in list_to_count:
        if id in ret_dict.keys():
            ret_dict[id] += 1
        else:
            ret_dict[id] = 1
    return ret_dict

left_count = count_list(left_list)
right_count = count_list(right_list)


score = 0
for left in left_list:
    if left in right_count.keys():
        score += left*right_count[left]
